...
class UnitConverter:
    ...
    def derive_operation_unit(self, op_name: str, *unit_strs: str) -> str:
        """
        Calculates the resulting unit from a mathematical operation.

        Parameters
        ----------
        op_name : str
            Name of the operation ('add', 'multiply', 'power', etc.)
        unit_strs : str
            Input unit strings
        ...
        -------
        str
            The resulting unit string, or "unitless"
        """
        if not self.is_enabled():
            # Basic string fallback if pint is not installed
            if op_name == 'multiply':
                # Handle scalar multiplication
                for u in unit_strs:
                    if u not in ('1', '1.0', 'dimensionless', ''):
                        return u
                return '1'
            if op_name in ('divide', 'true_divide'):
                # Handle scalar division
                if len(unit_strs) > 0 and unit_strs[0] not in ('1', '1.0', 'dimensionless', ''):
                    return unit_strs[0]
                return '1'
            return unit_strs[0] if unit_strs else "unitless"

        try:
            if op_name in ('add', 'subtract'):
                # Addition requires compatibility, return the first unit
                q0 = 1.0 * (self.parse_unit(unit_strs[0]) if unit_strs and unit_strs[0] else self.ureg.dimensionless)
                result_unit = str(q0.units)
                return self.simplify_derived_unit(result_unit)

            elif op_name == 'multiply':
                # Skip dimensionless and numeric inputs
                valid_strs = [
                    u for u in unit_strs 
                    if u not in (None, '', '1', '1.0', 'dimensionless') 
                    and not self._is_numeric_string(u)
                ]
                if not valid_strs:
                    return '1'
                if len(valid_strs) == 1:
                ...
                q0 = 1.0 * self.parse_unit(valid_strs[0])
                q1 = 1.0 * self.parse_unit(valid_strs[1]) if len(valid_strs) > 1 else self.ureg.dimensionless
                result_unit = str((q0 * q1).units)
                return self.simplify_derived_unit(result_unit)

            elif op_name in ('divide', 'true_divide'):
                # Skip dimensionless and numeric inputs
                q0_str = (
                    unit_strs[0] 
                    if len(unit_strs) > 0 
                    and unit_strs[0] not in (None, '', '1', '1.0', 'dimensionless')
                    and not self._is_numeric_string(unit_strs[0])
                    else None
                )
                q1_str = (
                    unit_strs[1] 
                    if len(unit_strs) > 1 
                    and unit_strs[1] not in (None, '', '1', '1.0', 'dimensionless')
                    and not self._is_numeric_string(unit_strs[1])
                    else None
                )
                
                if q0_str is None and q1_str is None:
                    return '1'
                if q0_str is None:
                    return f"1 / {q1_str}" if q1_str else '1'
                if q1_str is None:
                    return q0_str
                    
                q0 = 1.0 * self.parse_unit(q0_str)
                q1 = 1.0 * self.parse_unit(q1_str)
                result_unit = str((q0 / q1).units)
                return self.simplify_derived_unit(result_unit)

            elif op_name == 'power':
                # For power, unit_strs[1] is the numeric exponent, not a unit
                q0 = 1.0 * (self.parse_unit(unit_strs[0]) if len(unit_strs) > 0 and unit_strs[0] else self.ureg.dimensionless)
                exponent = float(unit_strs[1])
                result_unit = str((q0 ** exponent).units)
                return self.simplify_derived_unit(result_unit)

            elif op_name in ('sin', 'cos', 'tan', 'exp', 'log'):
                return "dimensionless"

            elif op_name == 'sqrt':
                q0 = 1.0 * (self.parse_unit(unit_strs[0]) if len(unit_strs) > 0 and unit_strs[0] else self.ureg.dimensionless)
                result_unit = str((q0 ** 0.5).units)
                return self.simplify_derived_unit(result_unit)

        except Exception as e:
            warnings.warn(f"Unit algebra failed for {op_name} on {unit_strs}: {e}")

        return "unitless"
    
    def simplify_derived_unit(self, unit_str: str) -> str:
        """
        Simplify a derived unit to the most common equivalent unit name.
        
        For example, converts "kilogram * meter ** 2 / ampere ** 2 / second ** 3" to "ohm".
        Uses pint's dimensionality database to find simpler named units.
        
        Parameters
        ----------
        unit_str : str
            Unit string from pint (e.g., "meter / second", "kilogram * meter ** 2 / second ** 3")
            
        Returns
        -------
        str
            Simplified unit string (e.g., "hertz", "ohm", "watt")
        """
        if not self.is_enabled():
            return unit_str
        ...
        try:
            # Parse the unit and get its dimensionality
            q = 1.0 * self.ureg(unit_str)
            target_dim = q.dimensionality
            
            # List of common derived units to check
            common_units = [
                'ohm', 'hertz', 'newton', 'pascal', 'joule', 'watt',
                'coulomb', 'volt', 'farad', 'siemens', 'weber', 'tesla',
                'henry', 'gray', 'becquerel', 'sievert', 'katal',
                'lux', 'lumen'
            ]
            
            # Check each common unit for dimensionality match
            for unit_name in common_units:
                try:
                    q_test = 1.0 * getattr(self.ureg, unit_name)
                    if q_test.dimensionality == target_dim:
                        return unit_name
                except (AttributeError, pint.UndefinedUnitError):
                    continue
            
            # If no match found, return the pint-formatted string
            return unit_str
            
        except Exception as e:
            warnings.warn(f"Could not simplify unit '{unit_str}': {e}")
            return unit_str
    
    def check_dimensional_compatibility(self, unit_str1: str, unit_str2: str) -> bool:
        """
        Check if two units are dimensionally compatible (can be added/subtracted).
        ...
        """
        if not self.is_enabled():
            # Without pint, assume compatibility
            return True
        
        try:
            u1 = self.parse_unit(unit_str1)
            u2 = self.parse_unit(unit_str2)
            ...
            # Check if dimensionalities are equal
            return q1.dimensionality == q2.dimensionality
            
        except Exception:
            # If can't check, assume compatible
            return True
