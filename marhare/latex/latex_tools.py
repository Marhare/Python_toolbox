"""LaTeX rendering helpers for scientific reporting.

Main capabilities:

- metrological rounding for ``value ± sigma`` formatting,
- scalar/vector/quantity rendering to LaTeX strings,
- configurable table generation,
- optional siunitx-compatible output,
- export helpers for ``.tex`` artifacts.
"""
...
def _extract_value_sigma(q):
    v, s = value_quantity(q)
    return np.asarray(v), np.asarray(s)


def _infer_symbol_from_stack(obj) -> str:
    """Best-effort local variable name inference for unnamed Quantity objects."""
...
def _resolve_symbol(mag) -> str:
    """Resolve display symbol, inferring from caller locals when absent."""
    if isinstance(mag, Quantity):
        explicit = mag.symbol or ""
        if explicit:
            return str(explicit)
        return _infer_symbol_from_stack(mag)
    return str(mag.get("symbol", "") or "")
...
def valor_pm(
    ...
    *magnitudes,
    ...
    # Flow: detect quantity-like dicts here, normalize to (value, sigma), then render.
    mags = None
    ...
    if is_quantity and not magnitudes:
        if sigma is None:
            ...
            if is_quantity_obj and hasattr(valor, "has_groups") and valor.has_groups():
                if siunitx:
                    raise ValueError(
                ...
                for group_name in group_names:
                    ...
                    v_g, s_g = _extract_value_sigma(group_q)
    ...
    if mags is not None:
        ...
        for mag in mags:
            ...
            values.append(v_arr)
            sigmas.append(s_arr)
            symbols.append(_resolve_symbol(mag))
...
def latex_quantity(
    ...
    **kw
    ...
    """
    Format one quantity or a sequence of quantities into LaTeX.

    Parameters
    ----------
    mag : dict | Quantity | list | tuple
        Single quantity-like object or a collection of quantities. For collections,
        a LaTeX table is generated by delegating to ``valor_pm``.
    cifras : int, default 2
        Number of significant decimal places used by formatter helpers.
    siunitx : bool, default False
        If True, uses ``siunitx``-compatible formatting where supported.
    **kw
        Extra keyword arguments forwarded to ``valor_pm``.

    Returns
    -------
    str
        LaTeX string for a single quantity or a LaTeX table for multiple inputs.

    Notes
    -----
    - Symbolic expressions are resolved upstream; this function renders numeric
      value/sigma/unit representations only.
    - Dimensionless units (for example ``"1"``) are suppressed in display helpers.
    """
